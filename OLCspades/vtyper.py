    ...
    def vtyper(self):
        ...
        for sample in self.metadata:
            if sample.general.stx:
                ...
                try:
                    os.symlink(sample.general.bestassemblyfile, '{}.fasta'.format(linkfile))
                # Except os errors
                except OSError as exception:
                    # If the os error is anything but directory exists, then raise
                    if exception.errno != errno.EEXIST:
                        raise
                sample.commands.famap = 'famap -b {}.famap {}.fasta'.format(linkfile, linkfile)
                sample.commands.fahash = 'fahash -b {}.hash {}.famap'.format(linkfile, linkfile)
                ...
                sample.commands.epcr = 're-PCR -S {}.hash -r + -m 10000 -n 1 -g 0 -G -q -o {}.txt {}'\
                    .format(linkfile, linkfile, sample[self.analysistype].primers)
                sample[self.analysistype].resultsfile = '{}.txt'.format(linkfile)
                self.epcrqueue.put((sample, linkfile))
        self.epcrqueue.join()
        self.epcrparse()
