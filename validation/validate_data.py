...
if __name__ == '__main__':
        ...
        for row in reader:
            ...
            for gene in expected_marker_genes[row['SeqID']]:
                if gene not in geneseekr_items:
                    logging.info('WARNING: Expected to find {gene} in {sample}, '
                                 'but {gene} was not found in GeneSeekr_Profile'.format(gene=gene,
                                                                                        sample=row['SeqID']))
                    warning_flag = True

            # Categorical verification: easy peasy.
            if expected_genera[row['SeqID']] != row['Genus']:
                logging.info('WARNING: Expected genus to be {expected_genus} for {sample}, '
                             'but got genus {found_genus}!'.format(expected_genus=expected_genera[row['SeqID']],
                                                                   sample=row['SeqID'],
                                                                   found_genus=row['Genus']))
                warning_flag = True
            if expected_contam[row['SeqID']] != row['SamplePurity']:
                logging.info('WARNING: Expected SamplePurity to be {expected_purity} for {sample}, '
                             'but got purity {found_purity}!'.format(expected_purity=expected_contam[row['SeqID']],
                                                                     sample=row['SeqID'],
                                                                     found_purity=row['SamplePurity']))
                warning_flag = True
            if expected_rmlst[row['SeqID']] != row['rMLST_Result']:
                logging.info('WARNING: Expected rMLST to be {expected_rmlst} for {sample}, '
                             'but got rMLST {found_rmlst}!'.format(expected_rmlst=expected_rmlst[row['SeqID']],
                                                                   sample=row['SeqID'],
            ...
            if n50 == 'ND':
                if n50_ranges[row['SeqID']][0] != 'ND':
                    logging.info('WARNING: Expected N50 did not match for {sample}. Found ND, but expected'
                                 ' {expected_n50}'.format(sample=row['SeqID'],
                                                          expected_n50=n50_ranges[row['SeqID']][0]))
                    warning_flag = True
            else:
                n50 = int(n50)
                if not n50_ranges[row['SeqID']][0] <= n50 <= n50_ranges[row['SeqID']][1]:
                    logging.info('WARNING: N50 for {sample} did not fall in expected range. N50 was {n50}, '
                                 'expected range is from {range_bottom} to '
            ...
            if not contig_ranges[row['SeqID']][0] <= num_contigs <= contig_ranges[row['SeqID']][1]:
                logging.info('WARNING: Number of contigs for {sample} did not fall in expected range. Number of contigs was {num_contigs}, '
                             'expected range is from {range_bottom} to '
                             '{range_top}'.format(sample=row['SeqID'],
            ...
            if not total_length_ranges[row['SeqID']][0] <= total_length <= total_length_ranges[row['SeqID']][1]:
                logging.info('WARNING: Total length for {sample} did not fall in expected range. Total length was {total_length}, '
                             'expected range is from {range_bottom} to '
                             '{range_top}'.format(sample=row['SeqID'],
    ...
    for i in range(1, len(lines)):
        if len(lines[i].split(',')) != correct_column_number:
            logging.info('WARNING: {sample} has {bad_column_number} columns when it should have {correct_column_number}'
                         '. Check its formatting, because something has gone '
                         'wrong!'.format(sample=lines[i].split(',')[0],
    ...
    if warning_flag:
        logging.info('All checks complete, one or more warning(s) encountered. COWBAT not successfully validated :(')
    else:
